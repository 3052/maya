...
// fMP4 streams the segment count is then written to the sidecar and the
// sample tables are saved in the output file itself via the moov that the
// stop writes, so running the same command again resumes. Streams written
// without remuxing are not resumable.
func orchestrateDownload(job *downloadJob) error {
   ...
   segmentsDone, err := executeDownload(job.allRequests[*segments:], key, remux, file, job.threads, stop)
   switch {
   case errors.Is(err, ErrStopped):
      // The only path that writes the sidecar.
      if err := writeResume(outputName+".resume", *segments+segmentsDone); err != nil {
         return err
      }
