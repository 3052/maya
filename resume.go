...
// the remuxer state from them. The sidecar is replaced atomically after
// every segment, so a crash at any point resumes from the last full
// segment. Streams written without remuxing are not resumable and get no
// sidecar.
...
func readResume(path string) (*int, error) {
   ...
   segments, err := strconv.Atoi(strings.TrimSpace(string(data)))
...
func writeResume(path string, segments int) error {
   tmp := path + ".tmp"
   if err := os.WriteFile(tmp, []byte(strconv.Itoa(segments)), 0o666); err != nil {
      return err
   }
