...
// and optionally downloads the representation matching the provided ID.
func Filter(response *http.Response, configVar *Config, id string) error {
   ...
   for _, group := range groups {
      ...
      if group[0].ID == id {
         return configVar.download(group)
      }
   }

   return fmt.Errorf("representation '%s' not found", id)
}
